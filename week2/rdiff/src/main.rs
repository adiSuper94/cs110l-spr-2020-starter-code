...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                let res = grid.set(i + 1, j + 1, grid.get(i, j).unwrap() + 1);
                match res {
                    Ok(()) => (),
                    Err(_err) => return grid,
                }
            } else {
                let res = grid.set(
                    i + 1,
                    j + 1,
                    if grid.get(i + 1, j).unwrap() > grid.get(i, j + 1).unwrap() {
...
fn main() {
    ...
    let seq1: Vec<String> = match read_file_lines(filename1) {
        Ok(vec) => vec,
        Err(err) => {
            println!("invalid file name for arg1 \n err: {}", err);
            return;
        }
    };
    let seq2: Vec<String> = match read_file_lines(filename2) {
        Ok(vec) => vec,
        Err(err) => {
            println!("invalid file name for arg2 \n err: {}", err);
            return;
        }
    };
